# Hi, I'm **Ayaan**

I'm a **student** and **researcher** passionate about creating tools and solving problems — from **websites** to **efficient neural networks**.

---

## What I'm Building
- **Developing neural networks** to support **efficiency** studies
- **Researching ML theory** to optimize computation
- **Maintaining and improving** websites for **nonprofit organizations**
- **Creating web tools** for **optimization** and **automation**

---

## What I'm Learning
Currently exploring:
- **Machine learning algorithms**  
- **Data analysis and manipulation**  
- **Calculus and Linear Algebra**

---

## Key Skills

### Programming & Tools
[![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=fff)](#)
[![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=000)](#)
[![HTML](https://img.shields.io/badge/HTML-%23E34F26.svg?logo=html5&logoColor=white)](#)
[![CSS](https://img.shields.io/badge/CSS-639?logo=css&logoColor=fff)](#)
[![LaTeX](https://img.shields.io/badge/LaTeX-00A0A0?logo=latex&logoColor=fff)](#)
[![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A22846?logo=raspberrypi&logoColor=white)](#)
[![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)](#)
[![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=fff)](#)

### **Collaboration** • **Leadership** • **Mentorship**

---

## Where to Find Me
- **Website**: [aynkhn.com](https://aynkhn.com)  
